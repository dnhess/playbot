...
impl Level {
    pub async fn find_by_guild_id_and_user_id(
        client: &Client,
        ...
        if let Ok(Some(ref level)) = result {
            if let Err(e) =
                crate::utils::cache_in_redis::<Self>(&key, &level, redis_conn, 300).await
            {
    ...
    pub async fn find_top_users_by_guild(
        redis_conn: &mut Connection,
        client: &Client,
        guild_id: &str,
        limit: i64,
        ...
        let serial: Vec<Self> = match futures::TryStreamExt::try_collect(cursor).await {
            Ok(serial) => {
                if let Err(e) =
                    crate::utils::cache_in_redis::<Vec<Self>>(&key, &serial, redis_conn, 3600).await
                {
