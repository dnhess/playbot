...
impl ReactionRole {
    ...
    pub async fn find_all_by_guild_id(
        client: &Client,
    ...
    pub async fn find_by_guild_id_and_message_id_and_emoji(
        client: &Client,
        redis_conn: &mut Connection,
        guild_id: &str,
        message_id: &str,
        ...
        if let Ok(Some(json)) = cached {
            if let Ok(reaction_roles) = serde_json::from_str::<Vec<Self>>(&json) {
                ...
                tracing::event!(target: "backend", tracing::Level::DEBUG, "{:?}", reaction_roles);
                let reaction_role = reaction_roles.into_iter().find(|reaction_role| {
                    reaction_role.message_id == message_id && reaction_role.emoji.trim() == emoji
                });
        ...
        match Self::find_all_by_guild_id(client, redis_conn, guild_id).await {
            Ok(reaction_roles) => {
                ...
                let role_key = format!(
                    "reaction_roles_for_guild:{}:{}:{}",
                    guild_id, message_id, emoji
                );

                if let Some(reaction_role) = reaction_role {
                    if let Err(e) =
                        crate::utils::cache_in_redis(&role_key, &reaction_role, redis_conn, 84600)
                            .await
                    {
